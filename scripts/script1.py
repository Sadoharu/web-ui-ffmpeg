...
def run_ffmpeg(stop_event, heartbeat_file):
    while not stop_event.is_set():
        ffmpeg_command = [
            ...
            '-c:v', 'copy', '-c:a', 'copy', '-f', 'mpegts', 'srt://localhost:5000'
        ]
        ...
        while True:
            ...
            if not line:
                break  # Завершити внутрішній цикл, якщо ffmpeg завершив вивід

            print(line, end='')
            if re.search('Decklink input buffer overrun', line):
                logging.error('Decklink input buffer overrun detected.')
                break  # Завершити внутрішній цикл для перезапуску ffmpeg
        ...
        process.terminate()
        process.wait()
        logging.info("ffmpeg process terminated. Attempting restart...")
        time.sleep(1)  # Затримка перед перезапуском

def main():
    stop_event = multiprocessing.Event()
    heartbeat_file = "heartbeat_script1.txt"  # Унікальна назва для кожного скрипта
